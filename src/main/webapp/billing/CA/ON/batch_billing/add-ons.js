 ...
 *          * jquery dataTables v.1.9.4
 * =========================================================
 * This document will be used to store js components of all
 * add-ons for the batch billing pages.
 * =========================================================
 */
...
!function( $ ) {
 ...
 "use strict";
  ...
  , transferAttributes: function() {
    ...
    this.$target.attr('id', this.$source.attr('id')+"_val") 
	...
	DateTimePicker.prototype={
		...
		hide:function(){
			...
			for(var i=0;i<collapse.length;i++){
				...
				if(collapseData&&collapseData.transitioning)return}this.widget.hide();
		...
		setStartDate:function(date){
			if(date instanceof Date){this.startDate=date}
		...
		place:function(){
			...
			offset.top=offset.top+this.height;var $window=$(window);
			if(this.options.width!=undefined){this.widget.width(this.options.width)}
		...
		fillDate:function(){
			...
			while(prevMonth.valueOf()<nextMonth){
				...
				clsName="";
				if(prevMonth.getUTCFullYear()<year||prevMonth.getUTCFullYear()==year&&prevMonth.getUTCMonth()<month){clsName+=" old"}
			...
			for(var i=0;i<12;i++){
				if(year==startYear&&startMonth>i||year<startYear){$(months[i]).addClass("disabled")}
				else if(year==endYear&&endMonth<i||year>endYear){$(months[i]).addClass("disabled")}
			}
			html="";year=parseInt(year/10,10)*10;
			var yearCont=this.widget.find(".datepicker-years").find("th:eq(1)").text(year+"-"+(year+9)).end().find("td");
			this.widget.find(".datepicker-years").find("th").removeClass("disabled");
			if(startYear>year){this.widget.find(".datepicker-years").find("th:eq(0)").addClass("disabled")}
					...
					switch(target[0].nodeName.toLowerCase()){
						case"th":switch(target[0].className){
							case"switch":this.showMode(1)
								break;
							case"prev":
							case"next":var vd=this.viewDate;
		...
		actions:{
			incrementHours:function(e){ this._date.setUTCHours(this._date.getUTCHours()+1)},
			incrementMinutes:function(e){this._date.setUTCMinutes(this._date.getUTCMinutes()+1)},
			incrementSeconds:function(e){this._date.setUTCSeconds(this._date.getUTCSeconds()+1)},
			decrementHours:function(e){this._date.setUTCHours(this._date.getUTCHours()-1)},
			decrementMinutes:function(e){this._date.setUTCMinutes(this._date.getUTCMinutes()-1)},
			decrementSeconds:function(e){this._date.setUTCSeconds(this._date.getUTCSeconds()-1)},
		...
		keypress:function(e){
			...
			if(!mask.pattern.test(val.slice(mask.start))){
				...
				while((mask=this._mask[this._maskPos])&&mask.character){ val+=mask.character; this._maskPos++}
				val+=c;
				if(mask.end!=val.length){ input.val(val); return false}
				else{
					if(!mask.pattern.test(val.slice(mask.start))){
		...
		_finishParsingDate:function(parsed){
			...
			if(parsed.Period12){
				if(/pm/i.test(parsed.Period12)){if(hours!=12)hours=(hours+12)%24}
				else{hours=hours%12}
			}
		...
		_compileFormat:function(){
			...
			while(match=formatComponent.exec(str)){
				...
				if(component in dateFormatComponents){
					...
					mask.push({
						pattern:new RegExp(dateFormatComponents[component].getPattern(this)),property:dateFormatComponents[component].property,
						start:pos,
		...
		_attachDatePickerEvents:function(){
			...
			if(this.options.maskInput){
				...
				if(this.options.maskInput){
					this.$element.on({
						keydown:$.proxy(this.keydown,this),
						keypress:$.proxy(this.keypress,this)},"input"
					)}
		...
		_detachDatePickerEvents:function(){
			...
			if(this.isInput){
				...
				if(this.options.maskInput){this.$element.off({keydown:this.keydown,keypress:this.keypress},"input")}
				if(this.component){this.component.off("click",this.show)}
				else{this.$element.off("click",this.show)}
			}},
		_detachDatePickerGlobalEvents:function(){
			$(window).off("resize.datetimepicker"+this.id);
			if(!this.isInput){$(document).off("mousedown.datetimepicker"+this.id)}
		},
		_isInFixed:function(){
			if(this.$element){
