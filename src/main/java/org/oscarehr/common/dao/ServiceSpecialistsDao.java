...
public class ServiceSpecialistsDao extends AbstractDao<ServiceSpecialists> {
	...
	public List<ServiceSpecialists> findBySpecId(Integer specId) {
		String sqlCommand = "select x from ServiceSpecialists x where x.specId=?1 ";

		Query query = entityManager.createQuery(sqlCommand);
		query.setParameter(1, specId);
		
		@SuppressWarnings("unchecked")
		List<ServiceSpecialists> results=query.getResultList();
